 ...
 *
 * Delivery is by email via Resend. If it fails, the full enquiry is written
 * to the server log so it can still be recovered by hand.
 *
...
export async function POST(req: Request) {
  ...
  try {
    ...
    //
    // Deliberately narrow. It requires a non-production build AND no Resend
    // key at all, so once a real key is present a genuine delivery failure
    // still surfaces as an error instead of hiding here.
    ...
    console.error("ENQUIRY LOST — email failed:", JSON.stringify(enquiry));
    return NextResponse.json(
